...
fig_1_tab %>%
  ...
  table_cell_font(row = 2:tab_nrow(.), column = 1,
                  color = "white", size = 7) %>% 
  table_cell_font(row = 2, column = 2:tab_ncol(.),
                  color = "#0099CC", size = 7) %>% 
  table_cell_font(row = 3, column = 2:tab_ncol(.),
                  color = "#999999", size = 7)

fig_1.d <-
fig_1_plot %>% 
  ggcuminc(theme = theme_cuminc(margin_title = margin(0,0, -35, 0))) +
  ...
  add_risktable(risktable_stats = "n.risk",
                ...
                theme = theme_risktable(margin_table = margin(5,0,0,0))) +
